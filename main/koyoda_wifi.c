...
static void wifi_start_task(void *arg)
{
    ...
    /* Let the already-working KOYODA UI settle before networking starts. */
    ...
    if (KOYODA_WIFI_SSID[0] == '\0')
    {
        ESP_LOGW(
            TAG,
            "No Wi-Fi credentials configured; KOYODA remains offline");
        s_started = false;
        vTaskDelete(NULL);
        return;
    }
    ...
    {
        ESP_LOGE(TAG, "Could not create Wi-Fi STA netif");
        s_started = false;
        vTaskDelete(NULL);
        return;
    }
    ...
    ESP_LOGI(TAG, "Background Wi-Fi enabled");
    vTaskDelete(NULL);
}
